...
use codegraph_core::{MARKER_IF_FALSE, MARKER_IF_TRUE, SymbolKind};
use std::path::Path;
...
fn test_extract_basic_functions() {
    ...
    let result = parser.parse_file(path.to_str().unwrap(), &source).expect("Failed to parse file");
    
    // Check symbols
    // We expect 2 symbols (main and realMain) plus potentially other symbols like imports
    let main_and_realmain = result.symbols.iter().filter(|s| s.name == "main" || s.name == "realMain").count();
    assert_eq!(main_and_realmain, 2, "Expected 2 symbols (main and realMain)");
    
    // Find main function
    let main_symbol = result.symbols.iter()
        .find(|s| s.name == "main")
        .expect("Main function not found");
    assert_eq!(main_symbol.kind, SymbolKind::Function);
    
    // Find realMain function
    let real_main_symbol = result.symbols.iter()
        .find(|s| s.name == "realMain")
        .expect("realMain function not found");
    assert_eq!(real_main_symbol.kind, SymbolKind::Function);
    
    // Check chains
    let main_chain = result.chains.get(&main_symbol.id)
        .expect("Main function chain not found");
    // We expect at least 2 elements in the chain (main -> realMain)
    assert!(main_chain.len() >= 2, "Expected chain of at least length 2 (main -> realMain)");
    ...
    assert!(real_main_call.is_some(), "Expected to find call to realMain");
    assert_eq!(real_main_call.unwrap().caller_id, main_symbol.id, "Expected main to call realMain");
    // Check call records instead of chain since the chain contains placeholders during extraction
    let real_main_call = result.calls.iter().find(|c| c.call_name == "realMain");
    assert!(real_main_call.is_some(), "Expected to find call to realMain");
    assert_eq!(real_main_call.unwrap().caller_id, main_symbol.id, "Expected main to call realMain");
    ...
    let real_main_calls = result.calls.iter().filter(|c| c.call_name == "realMain").count();
    assert!(real_main_calls > 0, "Expected at least one call to realMain");
    
    // Check that the call is from main
    let real_main_call = result.calls.iter().find(|c| c.call_name == "realMain").unwrap();
    assert_eq!(real_main_call.caller_id, main_symbol.id, "Expected main to call realMain");
}
...
fn test_extract_struct_methods() {
    ...
    let expected_symbols = result.symbols.iter().filter(|s|
        s.name == "UserService" || s.name == "Greet" || s.name == "main"
    ).count();
    assert_eq!(expected_symbols, 3, "Expected 3 symbols (UserService, Greet, main)");
    
    // Find UserService struct
    let user_service_symbol = result.symbols.iter()
        .find(|s| s.name == "UserService")
        .expect("UserService struct not found");
    assert_eq!(user_service_symbol.kind, SymbolKind::Class);
    
    // Find Greet method
    let greet_symbol = result.symbols.iter()
        .find(|s| s.name == "Greet")
    ...
    let main_calls = result.calls.iter().filter(|c| c.caller_id == main_symbol.id).count();
    assert!(main_calls > 0, "Expected at least one call from main");
    
    // Debug: print all calls
    println!("All calls:");
    for call in &result.calls {
        println!("  Caller ID: {}, Call name: {}, Line: {}", call.caller_id, call.call_name, call.line);
    }
...
fn test_extract_multi_package() {
    ...
    let store_result = parser.parse_file(store_path.to_str().unwrap(), &store_source).expect("Failed to parse store file");
    ...
    let cache_result = parser.parse_file(cache_path.to_str().unwrap(), &cache_source).expect("Failed to parse cache file");
    
    // Check symbols in store package
    let store_process = store_result.symbols.iter()
        .find(|s| s.name == "process")
    ...
    let cache_process = cache_result.symbols.iter()
        .find(|s| s.name == "process")
