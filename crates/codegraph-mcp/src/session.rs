...
pub enum OutputStyle {
    ...
    #[default]
    Minimize,
    /// Giữ key, lược bỏ field có value mặc định (None/0/""/[]/{}/false).
    Medium,
}
...
impl Session {
    ...
    /// → session chuyển sang `Ready`. `detail` là mức chi tiết mặc định cho
    /// symbol trong response các list tool (minimal/medium/verbose); `format` là
    /// output style (minimize/medium) — `None` giữ nguyên giá trị seed từ CLI.
    pub async fn init(
        &self,
        path: Utf8PathBuf,
        do_index: bool,
