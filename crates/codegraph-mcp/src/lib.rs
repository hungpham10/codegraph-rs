            ...
            return match name {
                "codegraph_doc_ingest" => {
                    ...
                    let format = args
                        ...
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string());
                    tools::dispatch_doc_ingest(doc_graph, path, format)
                        .await
                ...
                "codegraph_doc_search" => {
                    ...
                    tools::dispatch_doc_search(doc_graph, pattern, depth)
                        .await
                        .map_err(|e| McpError::internal_error(e.to_string(), None))
                        .map(|text| ToolOutput::Text {
                            text,
                ...
                "codegraph_doc_ingest_dir" => {
                    let path = args
                        ...
                        .ok_or_else(|| {
                            McpError::invalid_params(
                                "codegraph_doc_ingest_dir requires `path`",
                                None,
                            )
                        })?;
                    let limit =
                ...
                "codegraph_doc_remove" => {
                    let doc_id = args
                        ...
                        .ok_or_else(|| {
                            McpError::invalid_params(
                                "codegraph_doc_remove requires `doc_id`",
                                None,
                            )
                        })?;
